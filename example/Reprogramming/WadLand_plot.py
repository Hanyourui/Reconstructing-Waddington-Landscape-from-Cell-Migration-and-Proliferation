            ...
            for j in range(z_s_samples[sample_time[i]].shape[0]):
                if j not in not_v_list:
                    ...
                    arrow = patches.FancyArrow(arrow_start[0], arrow_start[1], v[sample_time[i]][j, 0] / v_scale,
                                               ...
                                               width=widths, color='k', head_width=0.05, head_length=0.035,
                                               shape='full', overhang=0.3, head_starts_at_zero=False)
                    ax1.add_patch(arrow)
        # 修改X轴和Y轴的标签字体大小和加粗
...
if __name__ == '__main__':
    ...
    if arguments.save_dir is not None:
        ...
        ckpt_path = os.path.join(arguments.save_dir, 'ckpt_itr1000.pth')
    ...
    # energy
    cell_type = []
    # all_type = ['0d', '4d', '7d',  '10d', '15d', '20d', 'iPSC']
    all_type = ['0d', '4d', '7d', '10d', '15d', '20d']
    ...
    for i in range(len(integral_time)):
        ax1.scatter(z_s_data[i][:, 0], z_s_data[i][:, 1], energy_np[i],
                    s=5 * 10, alpha=0.6, linewidth=0, facecolors=colors[i], edgecolors=colors[i], zorder=1)
    # add inferrred cell center trajecotry
    center_x = []
    center_y = []
    center_z = []
