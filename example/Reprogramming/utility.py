...
def input_args():
    arguments = Arguments()
    arguments.dataset = input("Name of the dataset. Option: embryo, etc. (default: embryo):") or "reprogramming"
    statepoints = input("State points of dataset (default: 1, 2, 3, 4, 5):")
    arguments.pseudo_time = [float(sp.strip())-1 for sp in statepoints.split(",")] if statepoints else [0, 1, 2, 3, 4, 5]
    arguments.num_iters = int(input("Number of the training iterations (default: 6000):") or 1000)
    arguments.num_samples = int(input("Number of sampling points per epoch: ") or 20)
    arguments.lr = float(input("Learning rate: ") or 3e-3)
    ...
    arguments.BRD_dim_hidden = int(input("Dimension of the hidden layer in BRDAutoencoder: ") or 10)
...
def train_model(mse, func, arguments, data_train, train_time, integral_time, sigma_now, options, device, itr):
    ...
    for i in range(len(train_time) - 1):
        ...
        loss = loss + L2_value1[0][i] * 1e2
        loss = loss + loss_x * 1e2
    ...
    if itr > 1:
        if ((itr % 100 == 0) and (itr <= arguments.num_iters - 400) and (sigma_now > 0.02) and (
